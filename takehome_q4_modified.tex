...
\begin{algorithm}[H]
	...
	\begin{algorithmic}[1]
		...
		\Function{getInfoGain}{Attributes$^*$,Target\_Attributes}
			...
			\For{all i in Attribute$^*$}\Comment $P(y=k | x_i = v_{ij}) = \frac{\theta_{ijk}P(y=k)}{P(x_i = v_{ij})}$
...
\begin{algorithm}[H]
	\caption{Testing Decision Tree}\label{Testing}
	\begin{algorithmic}[1]
		\Procedure{testTree}{Tree T, Datpoint D, missing\_probab}
		\State \Comment{Node is list that stores all the leaf nodes}
		\If{T.node == Leaf or D == $\phi$}
		\State \Return{T.node}
		\EndIf
		\For{all attributes A in D} \Comment{A is an attribute of D}
			\If{A is not in Tree.nodes}
				\State continue
			\Else
				\If{value(A) != $\phi$}
				\State d = Tree.checknode(value(A)) 
				
				\State \Comment{d is the decision taken at that note}
				\State Node.append\Big[\Call{testTree}(T.takepath(d), D-A \Big] \State \Comment{Take path along branch taken according to decision d}
				\ElsIf{value(A) == $\phi$}
					...
					\State Push decision tree in each branch of the node with\State probability $\mu_i$  
					\State Node.append(all the returned leaf node)
					\EndFor
				\EndIf
			\EndIf
			
			\If{Node.size == 1}
				\Return{Probability 1 for that class and zero for another}
			\Else
				\State S = Sum of probabilities for each class 
				\State\Comment{S is a list that class membership probabilities}
				\State \Return S
			\EndIf
