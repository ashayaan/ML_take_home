	...
	\begin{algorithmic}[1]
		\Procedure{BuildDecisionTree}{Dataset D,Target\_Attributes,Attributes}
			...
			\For {all possible values of A}
				...
				T.addNode\Big[BuildDecesionTree(subset,Target\_Attribures,Attributes-A)\Big]
			\EndFor
			\EndIf
		\EndProcedure
		
		\Procedure{getChiSquareScore}{Attributes}
		\State l = [] \Comment{Empty list initialization}
		\For{A in Attributes}
		\State $\mu_i = \frac{n}{n}$ \Comment{$n_i$ number of data points with $i^{th}$ label}
		...
		\Procedure{getInfo}{Attributes$^*$,Target\_Attributes}
